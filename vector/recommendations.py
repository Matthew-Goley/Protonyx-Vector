...
_TEMPLATES: dict[str, list[tuple[str, str]]] = {
    ...
    "SINGLE_POSITION": [
        (
            ...
            "Start splitting future deposits across 3–4 names in different sectors to build a real safety net.",
        ),
        (
            "Riding a single stock feels fine when it's green, but {single_ticker} is your only position right now.",
            "Your next few paychecks should go somewhere completely different — broad ETFs like VTI are a low-effort way to start.",
        ),
        (
            "{single_ticker} alone means your entire financial picture moves with one company's quarterly results.",
            "Even adding one or two unrelated stocks would dramatically reduce your all-or-nothing risk — aim for different sectors.",
        ),
        (
            ...
            "Look at adding positions in healthcare, energy, or consumer staples; the goal is to own things that don't all move together.",
        ),
        (
            "You're all-in on {single_ticker}, which means no diversification cushion if the stock has a bad year.",
            "Direct your next deposit into a different sector entirely — even a small second position starts building real diversification.",
        ),
        (
            "Everything is riding on {single_ticker} right now — that's a risk profile most advisors would flag immediately.",
            "Consider a broad-market ETF as your second position; it instantly gives you exposure to hundreds of companies at once.",
        ),
        (
            "{single_ticker} is doing the heavy lifting for your whole portfolio, which makes every earnings report feel high-stakes.",
            "Adding a second and third position in unrelated sectors turns those high-stakes moments into manageable events.",
        ),
        (
            "One stock means one set of risks — if {single_ticker}'s sector rotates out of favor, there's nowhere to hide.",
            "Your next buy should be in a sector you don't already own; think healthcare, financials, or a broad index.",
        ),
        (
            "Single-stock concentration isn't inherently wrong, but {single_ticker} is your only line of defense right now.",
            "Start building around it — even two or three other names in different industries changes your risk profile significantly.",
        ),
        (
            "All your eggs are in {single_ticker}'s basket — conviction is great but it shouldn't mean zero diversification.",
            "Your next deposit could go toward a low-cost index fund like VTI or VOO to start spreading that risk passively.",
        ),
        (
            "{single_ticker} is your entire portfolio, so its daily swings are your daily swings — there's no buffer anywhere.",
            "Think about opening a position in a defensive sector like utilities or consumer staples to create a shock absorber.",
        ),
        (
            "Owning just {single_ticker} is a high-conviction move, and conviction is fine — but the downside is completely uncushioned.",
            "Your next move should be diversification before performance; add something that zigs when {single_ticker} zags.",
        ),
        (
            "{single_ticker} could be a great stock, but no single company is worth your entire portfolio — the risk just isn't worth it.",
            "Consider splitting future deposits 50/50 between {single_ticker} and a broad ETF until you have a real base to build from.",
        ),
        (
            ...
            "Start diversifying now — even modest exposure to other sectors is enough to take the edge off any one company's bad news.",
        ),
        (
            "{single_ticker} is your whole world right now, and that's a tough spot to be in if anything goes wrong.",
            "The single best thing you can do for your portfolio today is add a position in a completely different industry.",
        ),
    ],
    ...
    "CONCENTRATED_STOCK": [
        (
            ...
            "Your next deposits should skip {concentrated_stock} entirely and go toward {underweight_sector} or a broad index to rebalance.",
        ),
        (
            "With {concentrated_stock_pct:.0f}% in {concentrated_stock}, one bad quarter from that company hits almost your whole portfolio.",
            "Look at adding {underweight_sector} exposure next — even a small position there starts reducing your concentration risk.",
        ),
        (
            ...
            "Consider directing your next few deposits elsewhere until {concentrated_stock} is a smaller share of the overall pie.",
        ),
        (
            ...
            "Your next buy should be in {underweight_sector} — spreading that weight out is the highest-leverage move you can make right now.",
        ),
        (
            ...
            "Start trimming that concentration over time — don't sell what's working, just stop adding and redirect new money elsewhere.",
        ),
        (
            ...
            "Your next deposit into {underweight_sector} would immediately reduce your single-stock risk without forcing you to sell anything.",
        ),
        (
            ...
            "Build out {underweight_sector} exposure with your next deposit to create some balance around your {concentrated_stock} position.",
        ),
        (
            ...
            "Even splitting future deposits 70/30 between other holdings and {concentrated_stock} starts correcting the imbalance over time.",
        ),
        (
            ...
            "You don't need to sell; just stop adding to {concentrated_stock} for a while and let your other positions catch up in weight.",
        ),
        (
            "{concentrated_stock} is a strong holding, but at {concentrated_stock_pct:.0f}% it's also your biggest unhedged risk.",
            "Adding {underweight_sector} exposure next would give you a buffer that doesn't correlate with {concentrated_stock}'s moves.",
        ),
        (
            ...
            "A diversification deposit into {underweight_sector} is the lowest-friction fix — no selling required, just redirecting new money.",
        ),
        (
            ...
            "Gradually build out other positions — your long-term goal should be no single stock above 20–25% of the total portfolio.",
        ),
        (
            "The {concentrated_stock_pct:.0f}% weight in {concentrated_stock} creates real scenario risk — a sector rotation alone could hurt.",
            "Direct your next deposit into {underweight_sector} to start rebalancing; it doesn't have to be dramatic to make a real difference.",
        ),
        (
            ...
            "A small position in {underweight_sector} would add a different return stream and reduce how much any single name controls your results.",
        ),
        (
            ...
            "Rebalance gradually by directing new cash to underweight areas like {underweight_sector} rather than adding more to {concentrated_stock}.",
        ),
    ],
    ...
    "CONCENTRATED_SECTOR": [
        (
            ...
            "Adding {underweight_sector} exposure next would immediately lower your sector risk without requiring you to sell anything.",
        ),
        (
            ...
            "Your next deposit into {underweight_sector} is the simplest way to start building a real buffer against sector-level swings.",
        ),
        (
            ...
            "Look at {underweight_sector} for your next buy — even modest exposure there gives you a return stream that doesn't rhyme with {concentrated_sector}.",
        ),
        (
            ...
            "Direct new money into {underweight_sector} to start building diversification across multiple sectors over time.",
        ),
        (
            ...
            "A position in {underweight_sector} would give you something that moves differently and reduces your reliance on one sector's performance.",
        ),
        (
            ...
            "Think about adding {underweight_sector} exposure — it doesn't need to be huge to meaningfully change your risk profile.",
        ),
        (
            ...
            "Your next buy in {underweight_sector} starts building the kind of diversification that protects you during sector rotations.",
        ),
        (
            ...
            "Adding {underweight_sector} gives you a hedge that doesn't move in lock-step with your existing holdings.",
        ),
        (
            ...
            "Consider {underweight_sector} for your next deposit — sector diversification is one of the few free lunches in investing.",
        ),
        (
            "Almost two-thirds of your portfolio is in {concentrated_sector} — strong conviction, but limited cushion if that changes.",
            "{underweight_sector} is underrepresented in your mix; even a small position there adds meaningful balance.",
        ),
        (
            ...
            "Start building {underweight_sector} exposure with your next deposit to create some sector balance over time.",
        ),
        (
            ...
            "A small {underweight_sector} position as your next buy starts to break that correlation.",
        ),
        (
            ...
            "Direct your next deposit into {underweight_sector} and let the sector diversification compound over time.",
        ),
        (
            ...
            "Spreading into {underweight_sector} next would give you a genuinely different return profile to complement what you already own.",
        ),
        (
            ...
            "Your next buy in {underweight_sector} doesn't need to be large — even 10–15% allocation there significantly reduces your concentration.",
        ),
    ],
    ...
    "DOWNTREND_HIGH_VOL": [
        ...
        (
            ...
            "New cash should wait on the sidelines or go toward {underweight_sector} rather than chasing falling names right now.",
        ),
        (
        ...
        (
            ...
            "Stay defensive: hold quality, review {biggest_loser}, and keep your next deposit in {underweight_sector} rather than doubling into weakness.",
        ),
        (
    ...
    "DOWNTREND_LOW_VOL": [
        (
            ...
            "{biggest_loser} is the weakest link at {biggest_loser_pct:+.1f}%; your next deposit could go into {underweight_sector} to start rebalancing.",
        ),
        (
            ...
            "Review {biggest_loser} and decide whether you're holding it for a reason or out of habit — then redirect new money into {underweight_sector}.",
        ),
        (
        ...
        (
            ...
            "Your next deposit should go to {underweight_sector} rather than adding to what's already underperforming.",
        ),
        (
        ...
        (
            ...
            "Redirect new cash to {underweight_sector} and stop adding to {biggest_loser} until it shows some sign of turning around.",
        ),
        (
            "Your portfolio is in a controlled slide — nothing dramatic, but the direction is wrong and has been for a while.",
            "Your best move is to add to {underweight_sector} with your next deposit and let the winning parts of your portfolio gain weight over time.",
        ),
        (
        ...
        (
            ...
            "Your next buy should be in {underweight_sector} to build some diversification while also adding weight to areas that aren't in a downtrend.",
        ),
        (
            ...
            "Trim {biggest_loser} to free up capital and redirect it somewhere with better momentum — {underweight_sector} is a reasonable next step.",
        ),
        (
            ...
            "A deposit into {underweight_sector} adds some balance and puts new money to work in an area that isn't already trending down.",
        ),
        (
        ...
        (
            ...
            "Act intentionally here: reduce {biggest_loser} if you can't defend the position, and build out {underweight_sector} with new money.",
        ),
        (
            ...
            "Your next deposit into {underweight_sector} is a constructive way to rebalance without having to sell anything outright.",
        ),
        (
            ...
            "Review your weakest positions and consider redirecting future deposits to {underweight_sector} where there's more upside potential.",
        ),
    ],
    ...
    "UPTREND_HIGH_VOL": [
        (
            ...
            "Don't chase your winners right now; your next deposit into something calmer like {underweight_sector} would smooth out the overall ride.",
        ),
        (
            ...
            "Consider adding {underweight_sector} with your next deposit — lower volatility names reduce the whipsaw without killing the upside.",
        ),
        (
            "Good returns, high risk — {biggest_gainer} is leading the charge but also contributing most to the volatility.",
            "Let the momentum run, but balance it out over time by directing new money toward lower-volatility areas like {underweight_sector}.",
        ),
        (
            ...
            "Lock in some stability by adding {underweight_sector} with your next deposit; you don't need to sell anything to start de-risking.",
        ),
        (
        ...
        (
            ...
            "Ride the winner but hedge the risk — your next deposit into {underweight_sector} creates a counterbalance without selling what's working.",
        ),
        (
            "High volatility in an uptrend is exciting but fragile — the same energy that drives gains can reverse quickly on bad news.",
            "Consider gradually building a lower-volatility position in {underweight_sector} to create a cushion if momentum fades.",
        ),
        (
            "Your portfolio is up {direction_slope:+.1f}% annualized with high volatility — strong performance with a bumpy ride.",
            "Don't add to your most volatile positions right now; direct new money into {underweight_sector} to lower the overall risk profile.",
        ),
        (
        ...
        (
            ...
            "Your next buy in {underweight_sector} adds diversification and lowers the portfolio's sensitivity to any single holding's swings.",
        ),
        (
            "Gains are real and the trend is your friend, but high volatility means those gains aren't fully secure yet.",
            "Build some defensive exposure in {underweight_sector} with your next deposit — it won't hurt your upside much and will cushion any pullback.",
        ),
        (
            ...
            "Diversify into {underweight_sector} over time so that if {biggest_gainer} hits a speed bump, the rest of the portfolio keeps moving.",
        ),
        (
            "Your portfolio is thriving — but the elevated volatility means one headline could erase weeks of gains in a day.",
            "Protect your progress: add lower-volatility exposure in {underweight_sector} and resist the urge to go all-in on what's been working.",
        ),
        (
            ...
            "Add some stability via {underweight_sector} next; you don't need to slow down the portfolio, just make the ride safer.",
        ),
    ],
    ...
    "UPTREND_LOW_VOL": [
        ...
        (
            ...
            "Keep your deposit cadence consistent and consider adding to {underweight_sector} to maintain balance as your portfolio grows.",
        ),
        (
        ...
        (
            ...
            "{biggest_gainer} has the most momentum; keep it running and fill in {underweight_sector} with your next deposit for balance.",
        ),
        (
        ...
        (
            ...
            "Your next deposit into {underweight_sector} would fill in an underweight area without disrupting what's already working.",
        ),
        (
        ...
        (
            ...
            "Add to {underweight_sector} with your next deposit if you want to be productive, otherwise just let it run.",
        ),
        (
        ...
        (
            ...
            "Your next dollar should go toward {underweight_sector} to maintain sector balance as the portfolio grows.",
        ),
        (
        ...
        (
            ...
            "If you want to put your next deposit to work productively, {underweight_sector} is the most underweight area in your mix.",
        ),
        (
    ...
    "NEUTRAL_HIGH_VOL": [
        (
            ...
            "This is a good time to rebalance toward lower-volatility holdings; your next deposit into {underweight_sector} would help calm things down.",
        ),
        (
            ...
            "Consider reducing your most volatile position or at minimum directing new cash into {underweight_sector} to lower the overall noise level.",
        ),
        (
            ...
            "The clearest improvement here is adding something calm and uncorrelated — {underweight_sector} exposure would reduce the choppiness immediately.",
        ),
        (
            "Sideways movement with elevated volatility is the least rewarding scenario — maximum uncertainty, minimum gain.",
            "Add {underweight_sector} with your next deposit to bring down the volatility while you wait for a clearer directional signal.",
        ),
        (
        ...
        (
            ...
            "Your next deposit should go into lower-volatility territory like {underweight_sector} to start tilting the risk-reward ratio in your favor.",
        ),
        (
            "You're flat with a {volatility_score}/100 volatility score — that's a lot of white-knuckle rides for zero net gain.",
            "Rebalance into calmer names over time and direct new cash to {underweight_sector} rather than adding to what's already volatile.",
        ),
        (
            ...
            "Use this period to rebalance: trim your most volatile name and redirect toward {underweight_sector} to smooth things out.",
        ),
        (
            ...
            "The fix is gradual: add lower-volatility positions in {underweight_sector} and let them dilute the choppiness over time.",
        ),
        (
            ...
            "A calm sector like {underweight_sector} as your next buy starts building the stability you need to ride out the rest.",
        ),
        (
            "Flat portfolio with a {volatility_score}/100 volatility score is a classic signal to rebalance, not just wait.",
            "Trim your most turbulent position slightly and redirect that capital to {underweight_sector} — it's a rebalance, not a retreat.",
        ),
        (
            ...
            "Instead, build toward {underweight_sector} exposure with your next deposit and let the calmer position provide some balance.",
        ),
        (
            ...
            "Redirect new cash to {underweight_sector} and let the lower-volatility position start pulling the overall risk profile down.",
        ),
        (
            "Sideways and noisy is a combination that benefits from patience and gentle rebalancing — not dramatic moves.",
            "Your next deposit into {underweight_sector} is the right kind of boring: it adds stability without requiring you to sell anything.",
        ),
    ],
    ...
    "NEUTRAL_LOW_VOL": [
        (
            ...
            "If you want to nudge growth, your next deposit could target {underweight_sector} or add to {biggest_gainer} which has the most momentum.",
        ),
        (
            "Low volatility, neutral direction — your portfolio is in a holding pattern but not losing ground.",
            "This is a great time to add to {underweight_sector} while prices are calm and you're not chasing momentum.",
        ),
        (
            ...
            "Use this window to build out {underweight_sector} exposure; adding to underweight areas when things are flat is low-risk and high-impact over time.",
        ),
        (
            "Flat and calm — your portfolio is holding its value without a lot of drama, which is actually a decent place to be.",
            "{biggest_gainer} has the best recent trend in your mix; your next deposit could reasonably go there or into {underweight_sector} to add balance.",
        ),
        (
            "A low-volatility, sideways portfolio is a blank canvas — nothing is demanding your attention, so you get to be intentional.",
            "Your next buy in {underweight_sector} adds sector balance and positions you ahead of any rotation that starts favoring that area.",
        ),
        (
            ...
            "Add to {underweight_sector} with your next deposit to fill in a gap in your sector coverage and improve long-term resilience.",
        ),
        (
            ...
            "Consider adding to {biggest_gainer} which has the most positive trend, or to {underweight_sector} to bring your sector allocation into better balance.",
        ),
        (
            ...
            "Your next deposit into {underweight_sector} adds a growth catalyst without increasing your overall risk profile.",
        ),
        (
            ...
            "Keep adding consistently; flat periods are just the time between growth spurts, and {underweight_sector} is a smart place to build during one.",
        ),
        (
            "The portfolio is calm and stable — that's not something to fix, it's something to build on.",
            "Your next deposit should go somewhere deliberate rather than just adding to existing positions — {underweight_sector} is underrepresented and worth adding.",
        ),
        (
            "Quiet and flat — you have the luxury of making a thoughtful decision rather than a reactive one right now.",
            "Either add to {biggest_gainer} to lean into your best momentum, or balance out via {underweight_sector} to improve your sector coverage.",
        ),
        (
            ...
            "A deposit into {underweight_sector} adds potential return streams and fills a gap in your diversification at the same time.",
        ),
        (
            "Your holdings are steady and unexciting — which is actually a healthy baseline to be building from.",
            "Use this calm period to add {underweight_sector} exposure; the best time to diversify is when you're not forced to.",
        ),
        (
            "Flat and quiet with low volatility — the portfolio isn't telling you to do anything urgent, so you get to act deliberately.",
            "Your next deposit into {underweight_sector} would add both diversification and exposure to an area you're currently light on.",
        ),
        (
            "Things are holding steady — no momentum in either direction, just a stable foundation to keep building from.",
            "Add to {underweight_sector} next to fill in a gap in your sector mix, or let {biggest_gainer} compound if you're happy with your current allocation.",
        ),
    ],
}
